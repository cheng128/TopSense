...
class Remap():
    ...
    def init_readfile(self):
        if self.args_type == 'test':
            word = self.args_word
            self.brt_file = f'../data/mapping_test_data/BRT_data_{word}_test.json'
    ...
    def init_savefile(self):
        ...
        if self.args_type == 'test':
            self.jsonl_file = f'{jsonl_base}test_jsonl/{filename}_{self.args_word}.jsonl'
        elif self.args_type == 'normal':
            self.jsonl_file = f'{jsonl_base}{filename}_all.jsonl'
...
def main():
    ...
    parser.add_argument('-w', type=str, default='bank')
