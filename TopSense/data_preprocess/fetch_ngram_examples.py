...
def fetch_ngrams(sentence_data):
...
def find_ngram_collocations(sentence_data, targetword):
    # calculate collocations of ngrams
    for sent, value in sentence_data.items():
        ngrams = value['ngrams']
        if ngrams:
            collocations = []
...
def fetch_examples(sentence_data):
    for sent, value in sentence_data.items():
...
def find_examples_collocations(sentence_data, targetword):
    for sent, value in sentence_data.items():
...
def main():
    ...
    for word_id, word_data in tqdm(word_examples.items()):
        if word_data['pos'] == 'verb':
            ...
            sentence_data = fetch_ngrams(sentence_data)
            sentence_data = find_ngram_collocations(sentence_data, targetword)

            sentence_data = fetch_examples(sentence_data)
            sentence_data = find_examples_collocations(sentence_data, targetword)

            word_examples[word_id]['examples'] = sentence_data

            with open('../data/boostrap_word_id.topics.examples.json', 'w') as f:
